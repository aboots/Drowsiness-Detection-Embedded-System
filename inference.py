...
import time
import torch
from client import handle_run_arguments, Client
...
def infer(ears_norm, mars_norm, pucs_norm, moes_norm):
    ...
    while cap.isOpened():
        ...
        if not success:
            print("Ignoring empty camera frame.")
            continue
        phone_model_result = 0
        if frame_before_run % 15 == 1:
            phone_model_result = infere_phone(image)
            print(f'phone model result = {phone_model_result}')
        ...
        frame_before_run += 1
        if frame_before_run >= 15 and len(input_data) == 20 and phone_model_result != 1:
            frame_before_run = 0
